...
def format_data(x_train, batch, c):
    import numpy as np
    if batch:
        data = x_train[:64]
    else:
...
def test_random_aug_func_with_apply_to_y_no_mag(func, batch, prob, apply_to_y, c, dataset):
    ...
    r = 10
    for _ in range(r):
        data = format_data(x_train, batch, c)
        ...
        if (prob == 0.0):
            assert tf.math.reduce_all(tf.equal(data, trans))


# @pytest.mark.skip()
@pytest.mark.parametrize("func", ['apply_random_rotate', 'apply_random_zoom', 'apply_random_x_zoom', 'apply_random_y_zoom',
                                  'apply_random_skew', 'apply_random_x_skew', 'apply_random_y_skew', 'apply_random_cutout'])
@pytest.mark.parametrize('batch', [True])
@pytest.mark.parametrize('prob', [0.0, 0.5, 1.0])
@pytest.mark.parametrize('mag', [0.0, 0.5, 1.0])
@pytest.mark.parametrize('apply_to_y', [True, False])
@pytest.mark.parametrize('c', [1, 3])
@pytest.mark.parametrize('dataset', [tf.keras.datasets.fashion_mnist, tf.keras.datasets.cifar10])
